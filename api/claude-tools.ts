...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    try {
      // Handle direct Claude-style tool calls
      const { function_name, parameters } = req.body;
      
      if (function_name === 'dice_roll') {
        const { notation, label, verbose } = parameters;
      ...
      if (function_name === 'dice_validate') {
        const { notation } = parameters;
      ...
      res.status(404).json({ error: `Unknown function: ${function_name}` });
    } catch (error) {
      res.status(500).json({ 
